...
treeNode* insertNode(treeNode* p, treeNode* node)
{
	...
	else if (node->student_id < p->student_id) //노드의 학번이 루트의 학번보다 작으면 왼쪽
		p->left = insertNode(p->left, node);
	else if (node->student_id > p->student_id) //노드의 학번이 루트의 학번보다 크면 오른쪽
...
void deleteNode(treeNode* root, treeNode* node)
{
	...
	{
		if (node->student_id < p->student_id) //입력한 학번이 더 작다면 parent는 p가되고 p는 왼쪽으로 간다
		{
			parent = p;
			p = p->left;
		}
...
int main()
{
	...
	while (1)
	{
		...
		case 2:
			...
			scanf(" %s %c %s %c %s %s", newnode->student_id, &newnode->department_id, newnode->department_name, &newnode->grade, newnode->birthday);
