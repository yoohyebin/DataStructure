...
void delete(Node** phead, Node* remove)
{
	Node* p = NULL;
	Node* prev;
	for (p = *phead, prev = p; p != NULL; p = p->next) //node의 처음부터 마지막 node까
	{
		if (p == remove) //지우려고하는 정보의 주소와 같을때
		{
			if (p->next == NULL) //지우려는 정보가 마지막이면
				prev->next = NULL; //지우려는 정보 앞의 다음을 NULL로 지정
			else
			{
				prev->next = p->next; //지우려는 정보 앞의 다음을 지우려는 정보의 다음으로 지정
			}
		}
	}
}
...
int main()
{
	...
	while (1)
	{
		...
		printf("원하는 기능을 입력하세요(1:검색 2:추가 3:삭제 4:종료)\n");
		scanf("%d", &menu);
		...
		case 1:
			...
			else if (a == 3)
			{
				for (curr = head; curr != NULL; curr = curr->next)
				{
					if (strcmp(curr->department_name, q) == 0)
						printf("%s	%c	%s	%c	%s	%s\n", curr->student_id, curr->department_id, curr->department_name, curr->grade, curr->name, curr->birthday);
				}
			}
			else if (a == 4)
